...
export function Dashboard() {
  ...
  const [countdown, setCountdown] = useState(() => {
    try { const v = Number(localStorage.getItem('setting_refresh_interval')); return [10, 60, 300, 600].includes(v) ? v : 60; } catch { return 60; }
  });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setFetchCompleted((prev: number) => prev + 1);
  ...
  // Reset countdown when interval changes or any fetch completes (success or failure)
  useEffect(() => {
    setCountdown(refreshInterval);
  }, [refreshInterval, fetchCompleted]);
  ...
  const exportBalancesCSV = useCallback(() => {
    ...
    const rows = sortedBalances.map((b: AtmBalance) => [
      ...
      b.balance_date ?? '',
      b.initial_balance_all ?? '',
      b.remaining_balance_all ?? '',
      b.initial_balance_all ? ((( b.remaining_balance_all ?? 0) / b.initial_balance_all) * 100).toFixed(1) : '',
      b.no_withdrawals_all ?? '',
    ]);
  ...
  const exportStatusCSV = useCallback(() => {
    ...
    const rows = statuses.map((s: AtmStatus) => [
      ...
      s.bil_cas6 ?? '',
      s.bil_cas7 ?? '',
      s.file_date ? new Date(s.file_date).toLocaleString() : '',
    ]);
    const csv = [headers, ...rows].map(r => r.join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-yellow-50">
      <header className="bg-gradient-to-r from-slate-900 via-yellow-900 to-slate-900 border-b border-yellow-800/50 shadow-lg">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 mt-2 flex-wrap">
                <p className="text-yellow-200">Real-time monitoring of ATM balances and hardware status</p>
                {isRefreshing && (
                ...
                {lastUpdated && !isRefreshing && (
                  <span className={`text-xs px-2 py-1 rounded ${isStale ? 'bg-red-700/60 text-red-200' : 'bg-yellow-800/40 text-yellow-300'}`}>
            ...
            <div className="mt-6 sm:mt-0 flex flex-col items-end gap-2">
              <div className="flex items-center gap-3 bg-yellow-800/40 backdrop-blur-sm px-4 py-3 rounded-lg border border-yellow-700/50">
                <Clock className="w-5 h-5 text-yellow-300" />
                <span className="text-sm font-medium text-yellow-200 hidden xs:inline">Refresh:</span>
