...
import { AtmBalance, AtmStatus, ComparisonResult, fetchLatestBalances, fetchLatestStatuses, fetchComparison, BALANCE_THRESHOLD_CRITICAL, BALANCE_THRESHOLD_LOW } from '../lib/api';
...
import { Search, AlertTriangle, CheckCircle, Activity, Clock, Zap, TrendingUp, Eye, EyeOff, WifiOff, Download, ChevronDown, ArrowUpDown, FileText } from 'lucide-react';
...
export function Dashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [balanceData, statusData, comparisonData] = await Promise.all([
        fetchLatestBalances(),
        fetchLatestStatuses(),
        // Comparison is an enhancement — don't fail the whole refresh if it errors
        fetchComparison(thresholdRef.current.critical, thresholdRef.current.low).catch(() => null),
      ]);

      if (comparisonData) setComparison(comparisonData);

      if (balanceData?.length > 0) {
        // "Previous" means the previous FILE, not the previous poll — otherwise
        // refill/drop deltas vanish on the next refresh of the same file
        const isNewFile = snapshotManager.isNewFile(balanceData[0].file_name ?? '');
        if (isNewFile && balancesRef.current.length > 0) {
        ...
        if (snapshotManager.isNewFile(fileName)) {
          ...
          const hwStats = (statusData ?? []).reduce((acc: { healthy: number; errors: number; warnings: number }, s: AtmStatus) => {
            const decoded = [
              ...
              decodeDeviceStatus(s.print_user), decodeDeviceStatus(s.door),
              decodeDeviceStatus(s.card_bin), decodeDeviceStatus(s.rej_bin),
  ...
  // ── Re-fetch comparison when thresholds change (debounced while typing) ───
  // Tracks the last-fetched values so mount/StrictMode re-runs don't refetch.
  const comparisonThresholdsRef = useRef({ critical: thresholdCritical, low: thresholdLow });
  useEffect(() => {
    const last = comparisonThresholdsRef.current;
    if (last.critical === thresholdCritical && last.low === thresholdLow) return;
    const t = setTimeout(() => {
      comparisonThresholdsRef.current = { critical: thresholdCritical, low: thresholdLow };
      fetchComparison(thresholdCritical, thresholdLow)
        .then(setComparison)
        .catch(() => { /* keep last good comparison */ });
    }, 500);
    return () => clearTimeout(t);
  }, [thresholdCritical, thresholdLow]);
  ...
  useEffect(() => {
    ...
    // Notification API is unavailable in some browsers/webviews and insecure contexts
    if (newlyGoneCritical.length > 0 && prevCriticalRef.current.size > 0 && typeof Notification !== 'undefined') {
  ...
  const errorTypeData = useMemo(() => {
    ...
    statuses.forEach((status: AtmStatus) => {
      const components = [
        ...
        { name: 'print_user', value: status.print_user },
        { name: 'door', value: status.door },
        { name: 'encryptor', value: status.encryptor },
        { name: 'card_bin', value: status.card_bin },
        { name: 'rej_bin', value: status.rej_bin },
        { name: 'depository', value: status.depository },
        { name: 'bil_cas1', value: status.bil_cas1 },
        { name: 'bil_cas2', value: status.bil_cas2 },
        { name: 'bil_cas3', value: status.bil_cas3 },
        { name: 'bil_cas4', value: status.bil_cas4 },
        { name: 'bil_cas5', value: status.bil_cas5 },
        { name: 'bil_cas6', value: status.bil_cas6 },
        { name: 'bil_cas7', value: status.bil_cas7 },
      ];
  ...
  const errorTypeChartData = useMemo(() =>
    errorTypeData
      .map((item: { name: string; value: number }) => ({
        ...item,
        name: ERROR_TYPE_LABELS[item.name] ?? item.name.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()),
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-yellow-50">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 mt-2 flex-wrap">
                ...
                {balances[0]?.file_name && (
                  <span className="inline-flex items-center gap-1.5 text-xs px-2 py-1 rounded bg-black/10 text-black/70 font-medium" title="Current report file">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-md p-6 border border-slate-100">
            ...
            {errorTypeChartData.length > 0 ? (
              <div>
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={errorTypeChartData} layout="vertical" margin={{ left: 8, right: 24 }}>
                    <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#e2e8f0" />
                    <XAxis type="number" allowDecimals={false} tick={{ fontSize: 12 }} />
                    <YAxis type="category" dataKey="name" width={90} tick={{ fontSize: 12 }} />
                    <Tooltip formatter={(value) => [`${value} ATMs`, 'Critical']} cursor={{ fill: '#f8fafc' }} />
            ...
            <label className="flex items-center gap-2 text-sm text-slate-600">
              ...
              <input
                ...
                onChange={(e: { target: HTMLInputElement }) => {
                  ...
                  if (Number.isNaN(val)) return; // empty/partial input — keep last valid value
        ...
        <div>
          ...
          <p className="text-sm text-slate-600 mb-6">Click a row to expand · Click a problem chip or column header to filter · Esc clears filters</p>
          <AtmStatusTable
      ...
      {selectedAtm && (
        <AtmDetailsModal
          balance={selectedAtm}
          status={
            // Guard each key — undefined === undefined would match the wrong ATM
            (selectedAtm.terminal_id != null
              ? statuses.find((s: AtmStatus) => s.atm_pid === selectedAtm.terminal_id)
              : undefined) ||
            (selectedAtm.atm_id != null
              ? statuses.find((s: AtmStatus) => s.atm_pid === selectedAtm.atm_id)
              : undefined) ||
            (selectedAtm.atm_name != null
