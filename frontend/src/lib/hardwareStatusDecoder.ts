...
export function decodeAtmStatus(statusCode: string | null | undefined): DecodedDeviceStatus | null {
...
export function decodeNetStatus(statusCode: string | null | undefined): DecodedDeviceStatus | null {
  if (!statusCode) return null;
  const trimmed = statusCode.trim().toUpperCase();
  if (!trimmed) return null;
...
export function decodeDeviceStatus(statusCode: string | null | undefined): DecodedDeviceStatus | null {
  // Only missing data is undecodable — an empty string is a trimmed blank,
  // which the spec defines as "device fully functional" (handled below)
...
export function decodeField(raw: string | null | undefined, fieldKey: string): DecodedDeviceStatus | null {
